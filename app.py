...
def hello_world():
    ...
    if request.form.get('signup'):
        form = RegistrationForm(request.form)
        # print(request.form.get('signup'), request.form.get('login'))
        if request.method == 'POST' and form.validate():
            ...
            print(f"insert into users(fname, lname, email_id, username, password, contact_no) values('{userdet['fname']}', '{userdet['lname']}', '{userdet['email']}', '{userdet['username']}', '{userdet['password']}', {userdet['phno']})")
        ...
        if request.method == 'POST' and form1.validate():
            ...
            ulist = cur.fetchall()[0]
            if bcrypt.check_password_hash(ulist[5], p):
                flash('Login successful!', 'success')
            else:
                flash('Could not find matching username or password. Please check your login credentials.', 'danger')
            # print(ulist[5], bcrypt.check_password_hash(ulist[5], p))
        print('wxyz')
    return render_template('login.html', pageType = str(request.url_rule), form = form)
